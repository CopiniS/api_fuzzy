...
class PotassioFuzzyG:
    ...
    def fazCalculo(ctc_entrada: str, potassio_entrada: str) -> float:
        ...
        potassio_por_ha['muito_baixo'] = fuzz.zmf(x_potassio_por_ha, 10, 30)
        potassio_por_ha['baixo'] = fuzz.trapmf(x_potassio_por_ha, [20, 40, 50, 70])
        potassio_por_ha['medio'] = fuzz.trapmf(x_potassio_por_ha, [60, 80, 90, 110])
        potassio_por_ha['alto'] = fuzz.trapmf(x_potassio_por_ha, [90, 110, 120, 130])
        potassio_por_ha['muito_alto'] = fuzz.trapmf(x_potassio_por_ha, [120, 130, 135, 140])
        ...
        potassio_sim.input['ctc'] = float(ctc_entrada)  
        potassio_sim.input['teor_potassio'] = float(potassio_entrada)  
